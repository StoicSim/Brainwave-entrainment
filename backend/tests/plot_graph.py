...
def start_live_plot():
    ...
    for ax, band in zip(axes, band_buffers.keys()):
        ax.set_title(band)
        ax.set_ylim(0, 500000)  # default max, will autoscale
    ...
    def animate(frame):
        ...
        for band, line in lines.items():
            ...
            x = times[-len(y):]
            # Align x-axis to seconds elapsed
            ...
            if y:
                min_y = min(y)
                max_y = max(y)
                if max_y - min_y < 10:  # prevent zero range
                    max_y += 10
                    min_y -= 10
